...
func main() {
	...
	req := jtrpc.NewRequest(context.Background(), "/echo/echo")
	...
	req = jtrpc.NewRequest(context.Background(), "/yes/yes")
	req.SetBodyString("no and yes")
	resp = <-must(client.Send(req)).Chan()
	resp.Headers.Parse()
	fmt.Println("Response:", resp)
	...
	req = jtrpc.NewRequest(context.Background(), "/no/no")
	req.SetBodyString("no and yes")
	resp = <-must(client.Send(req)).Chan()
	resp.Headers.Parse()
	fmt.Println("Response:", resp)
	...
	req = jtrpc.NewRequest(context.Background(), "/stream/stream")
	req.SetStream(true)
