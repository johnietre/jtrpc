...
impl Struct {
    ...
    fn to_lang(&self, lang: Lang) -> String {
        use Lang::*;
        match lang {
            Go => self.to_go(),
            Julia => self.to_julia(),
    ...
    fn to_go(&self) -> String {
        format!(
            "type {} struct {{\n{}\n}}",
            self.name,
            self.fields.iter().map(|f| f.to_go()).collect::<Vec<_>>().join("\n"),
        )
    }
    ...
    fn to_julia(&self) -> String {
        format!(
            "mutable struct {}\n{}\nend",
            self.name,
            self.fields.iter().map(|f| f.to_julia()).collect::<Vec<_>>().join("\n"),
        )
    }
    ...
    fn to_rust(&self) -> String {
        format!(
            "struct {} {{\n{}\n}}",
            self.name,
            self.fields.iter().map(|f| f.to_rust()).collect::<Vec<_>>().join("\n"),
        )
    }

    fn make_ser_deser_funcs(&self, lang: Lang) -> (String, String) {
        use Lang::*;
        match lang {
            Go => self.to_go(),
            Julia => self.to_julia(),
    ...
    fn make_go_ser_deser_funcs(&self) -> (String, String) {
    }

    fn make_julia_ser_deser_funcs(&self) -> (String, String) {
    }
...
impl Field {
    ...
    fn to_go(&self) -> String {
        format!("{} {}", self.name, self.type_.to_go())
    }

    fn to_julia(&self) -> String {
        format!("{}::{}", self.name, self.type_.to_julia())
    }

    fn to_rust(&self) -> String {
        format!("{}: {}", self.name, self.type_.to_rust())
    }

    fn make_ser_deser(&self, lang: Lang) -> String {
        use Lang::*;
        match lang {
            Go => self.to_go(),
            Julia => self.to_julia(),
    ...
    fn make_go_ser_deser_line(&self) -> String {
        let (ser, deser) = self.type_.make_go_ser_deser();
        (
            format!("", self.name, ser),
            format!("x.{} = {}", self.name, deser),
        )
    }
}
...
impl Type {
    ...
    fn to_go(&self) -> String {
        ...
        match self {
            ...
            Bool => "bool".into(),
            Str(_, _) => "string".into(),
            Array(t, None) => format!("[]{}", t.to_go()),
            Array(t, s) => format!("[{s}]{}", t.to_go()),
            Map(k, v, _) => format!("map[{}]{}", k.to_go(), v.to_go()),
            Struct(name) => name.to_string(),
            Optional(t) => format!("*{}", t.to_go()),
        }
    }
    ...
    fn to_julia(&self) -> String {
        ...
        match self {
            ...
            F64 => "Float64".into(),
            Bool => "Bool".into(),
            Str(_, _) => "AbstractString".into(),
            Array(t, _) => format!("Vector{{{}}}", t.to_go()),
            Map(k, v, _) => format!("Dict{{{}, {}}}", k.to_go(), v.to_go()),
            Struct(name) => name.to_string(),
            Optional(t) => format!("Union{{{}, Nothing}}", t.to_go()),
        }
    }
    ...
    fn to_rust(&self) -> String {
        ...
        match self {
            ...
            Array(t, None) => format!("[{}]", t.to_go()),
            Array(t, s) => format!("[{}; {s}]", t.to_go()),
            Map(k, v, _) => format!("HashMap<{}, {}>", k.to_go(), v.to_go()),
            Struct(name) => name.to_string(),
            Optional(t) => format!("Option<{}>", t.to_go()),
        }
    }

    fn make_ser_deser(&self, lang: Lang) -> (String, String) {
    }

    fn make_go_ser_deser(&self) -> (String, String) {
        use Type::*;
        match self {
            I(n) => todo!(),
            U(n) => todo!(),
