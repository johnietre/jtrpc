use crate::{utils::place8, ArcRes, Client, JtRPCError, Request, WriteUnpin, BV};
use std::borrow::Cow;
...
impl<W: WriteUnpin + 'static> Clone for Stream<W> {
    fn clone(&self) -> Self {
...
#[derive(Debug)]
pub(crate) struct InnerStream<W: WriteUnpin + 'static> {
    client: Client<W>,
    req: Request,
...
impl<W: WriteUnpin + 'static> InnerStream<W> {
    ...
    async fn recv_timeout(&self, dur: Duration) -> ArcRes<Message> {
        ...
        match rx.try_recv() {
            ...
            Err(TryRecvError::Disconnected) => {
                Err(self.get_closed_err_or(JtRPCError::StreamClosed(None)))
            }
    ...
    async fn send_msg(&self, mut msg: Message) -> ArcRes<()> {
        ...
        place8(&mut buf, self.req.id);
        buf[8] = msg.flags;
        if msg.body.len() != 0 {
            place8(&mut buf[9..], msg.body.len() as u64);
        }
        writer
            .write_all(&buf)
            .await
            .map_err(|e| Arc::new(JtRPCError::from(e)))?;
        writer
            .write_all(&msg.body)
            .await
            .map_err(|e| Arc::new(JtRPCError::from(e)))?;
...
impl<W: WriteUnpin + 'static> Drop for InnerStream<W> {
    fn drop(&mut self) {
        let ptr = self.close_err.load(Ordering::Relaxed);
        if !ptr.is_null() {
            // Safety
