...
use std::sync::{atomic::{AtomicBool, AtomicPtr, AtomicU64, Ordering}, Arc};
use tokio::io::{AsyncReadExt, AsyncWriteExt};
use tokio::net::{tcp::{OwnedReadHalf, OwnedWriteHalf}, TcpStream};
use tokio::sync::{mpsc::{self, UnboundedSender}, oneshot::{self, Sender, Receiver}, Mutex};

type ReqTup<R, W> = (Request, Sender<Response<R, W>>);
...
pub struct Client<R: ReadUnpin + 'static, W: WriteUnpin + 'static>(pub(crate) Arc<InnerClient<R, W>>);
...
impl<R: ReadUnpin + 'static, W: WriteUnpin + 'static> Client<R, W> {
    pub async fn with_rw(reader: R, writer: W) -> Result<Self, JtRPCError> {
        Ok(Self(InnerClient::with_rw(reader, writer).await?))
    }

    pub async fn send(&self, req: Request) -> Res<RespChan<R, W>> {
...
impl<R: ReadUnpin + 'static, W: WriteUnpin + 'static> Clone for Client<R, W> {
    fn clone(&self) -> Self {
        Self(Arc::clone(&self.0))
    }
}

// TODO: Store error from running
pub(crate) struct InnerClient<R: ReadUnpin + 'static, W: WriteUnpin + 'static> {
    pub(crate) writer: Mutex<W>,
    is_closed: AtomicBool,
    req_counter: AtomicU64,
    reqs: Mutex<HashMap<u64, ReqTup<R, W>>>,
    streams: Mutex<HashMap<u64, (UnboundedSender<Message>, Stream<R, W>)>>,
    close_err: AtomicPtr<Arc<JtRPCError>>,
}

impl<R: ReadUnpin + 'static, W: WriteUnpin + 'static> InnerClient<R, W> {
    pub async fn with_rw(mut reader: R, mut writer: W) -> Result<Arc<Self>, JtRPCError> {
        writer.write_all(INITIAL_BYTES_VER).await?;
        let resp = Response::<R, W>::read_from(&mut reader).await?;
        ...
        let _ = Arc::clone(&client).run(reader);
        Ok(client)
    }

    async fn run(self: Arc<Self>, mut reader: R) -> Res<()> {
        tokio::spawn(async move {
            loop {
                ...
                if let Some(req_tup) = self.reqs.lock().await.remove(&id) {
                    if let Err(e) = self.handle_resp(&mut reader, req_tup, &mut buf).await {
                            let _ = self.close_with_err(Some(e));
                            break;
                    }
                }
    ...
    async fn handle_resp(
        self: &Arc<Self>, reader: &mut R, (req, tx): ReqTup<R, W>, buf: &mut Vec<u8>,
    ) -> Res<()> {
        let flags = buf[8];
        ...
        let stream = if has_stream {
            ...
            self.streams.lock().await.insert(req_id, (tx, stream.clone()));
            Some(stream)
        } else {
            None
        };
    ...
    async fn handle_stream_msg(
        self: &Arc<Self>,
        reader: &mut R,
        (tx, stream): &(UnboundedSender<Message>, Stream<R, W>),
        mut buf: BV,
    ) -> Res<()> {
        let flags = buf[8];
    ...
    async fn send(&self, mut req: Request) -> Result<RespChan<R, W>, JtRPCError> {
        if self.is_closed() {
...
pub type RespChan<R, W> = Receiver<Response<R, W>>;
